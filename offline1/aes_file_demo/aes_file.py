...
def dechunk_file(states):
    text = dechunk_text(states)
    filename, _, hex_str = text.partition(r"\n")
...
def aes_decrypt_file(cipher, key):
    keys = expand_key(key)
    states = inverse_aes_rounds(cipher, keys)
    filename, content = dechunk_file(states)
    return filename, content
...
if __name__ == "__main__":
    ...
    filname, content = aes_decrypt_file(cipher, key)
